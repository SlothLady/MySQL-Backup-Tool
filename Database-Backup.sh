...
slack_message() {
	if [[ "${dry_run}" = false && "${SLACK_INTEGRATION}" = true ]]; then
		...
		curl -X POST "${SLACK_WEBHOOK_URL}" -H 'Content-Type: application/json' -d '{"attachments":[{"color":"'"${4}"'","blocks":[{"type":"header","text":{"type":"plain_text","text":"'"${1}"'","emoji":true}},{"type":"section","fields":[{"type":"mrkdwn","text":"*Status:*\n'"${3}"'"},{"type":"mrkdwn","text":"*Config File:*\n'"${2}"'"}]},{"type":"section","fields":[{"type":"mrkdwn","text":"*Hostname:*\n'"$(uname -n)"'"},{"type":"mrkdwn","text":"*Backup Filename:*\n'"${filename}"'"}]},{"type":"section","text":{"type":"mrkdwn","text":"*SHA256 Hash:*\n'"${filehash}"'"}}]}]}' >/dev/null 2>&1
		unset filehash
		unset filename
	fi
}
...
run_backup() {
	...
	if ! ssh -o BatchMode=yes -o ConnectTimeout=5 "${REMOTE_USER}@${REMOTE_HOST}" 'echo "Login successful."' >/dev/null 2>&1
	then
	...
	if ! echo "Hello ${REMOTE_HOST}" | ssh -o BatchMode=yes -o ConnectTimeout=5 "${REMOTE_USER}@${REMOTE_HOST}" "REMOTE_PATH=${REMOTE_PATH} && cat > \${REMOTE_PATH}/test"
	then
	...
	if ! mysql --defaults-file="${mysql_conf_path}" -e "SELECT 1;" >/dev/null 2>&1
	then
	...
	if ! mysql --defaults-file="${mysql_conf_path}" -e "USE ${MYSQL_DATABASE};" >/dev/null 2>&1
	then
	...
	if ! mysqldump --defaults-file="${mysql_conf_path}" --single-transaction --databases "${MYSQL_DATABASE}" 2>/dev/null | gzip -9 >"${BACKUP_PATH}/${BACKUP_FILENAME}"
	then
	...
	if ! scp "${BACKUP_PATH}/${BACKUP_FILENAME}" "${REMOTE_USER}"@"${REMOTE_HOST}":"${REMOTE_PATH}"
	then
	...
	if ! rm "${BACKUP_PATH}/${BACKUP_FILENAME}"
	then
